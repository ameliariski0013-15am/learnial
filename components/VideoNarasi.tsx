...
import { Youtube, Search, Volume2, VolumeX, ExternalLink, Copy } from 'lucide-react'
    ...
    <div className="max-w-3xl">
      <div className="bg-brand-50 border border-brand-100 rounded-2xl p-4 mb-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[12px] text-brand-400">Rekomendasi video YouTube + penjelasan audio dari materi</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-4">
        <textarea
          ...
          placeholder="Masukkan topik atau paste materi kuliah..."
          className="w-full min-h-[80px] p-3 border border-gray-100 rounded-xl text-[13px] bg-gray-50 resize-y focus:outline-none focus:border-brand-400 text-gray-700"
        />
        <div className="flex gap-2 mt-3">
          <button onClick={doVideo} disabled={loading || !text.trim()}
            ...
            {loading ? <><span className="dot-1">●</span><span className="dot-2">●</span><span className="dot-3">●</span> Mencari...</> : <><Search size={14} /> Cari Video & Buat Narasi</>}
          </button>
          {sharedText && (
